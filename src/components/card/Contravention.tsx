...
const ContraventionTable = ({ invoices, openConventionTable }) => {
  ...
  return (
    <div className="w-full  bg-white rounded-[1.25rem] py-[1.5rem] px-5">
      ...
      <table className="w-full border border-solid border-[#C5D5F8] rounded-[12px] border-collapse-separate ">
        <thead className="border-b border-b-[#C5D5F8]">
          <tr className="border-b border-b-[#C5D5F8] ">
            <th
              className={`text-[#757575] px-3 py-[0.75rem]  text-left   ${groteskText.className}`}
            >
              Contravention Type
            </th>
            <th
              className={`text-[#757575] px-4 py-[0.75rem]  text-left   ${groteskText.className}`}
            >
              Vehicle Reg Number
            </th>
            <th
        ...
        <tbody>
          {itemDetails.contravention.slice(0, visibleCount).map((invoice) => (
            <tr key={invoice.ticket} className="">
              <td
                className={`text-[#212121] px-[0.75rem] py-[0.75rem] ${groteskText.className}`}
              >
                {invoice.ticket}
              </td>
              <td
